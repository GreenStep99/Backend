...
public class VerifiedEventHandler {
    ...
    public Map<String, Object> sendMail(VerifiedEvent verifiedEvent) throws MessagingException {
        ...
        String content = "인증하신 ";
        if(verifiedEvent.getVerification()==DONE) {title +="완료되었습니다.";
            content += "[" + verifiedEvent.getSubmitMission().getMission().getMissionName() + "]가(이) 성공적으로 인증되었습니다!";}
        if(verifiedEvent.getVerification()==REJECTED) {title +="실패하였습니다.";
            content += "[" + verifiedEvent.getSubmitMission().getMission().getMissionName() + "]가(이) 인증에 실패하였습니다.\n 인증 실패 사유: " + verifiedEvent.getInfo() + "\n 다시 인증해주세요!";}
